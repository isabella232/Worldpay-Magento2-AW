...
class CaptureRequest implements ObserverInterface
{
    ...
    public function execute(\Magento\Framework\Event\Observer $observer)
    {
        ...
        $this->wplogger->info('came to capture22222');
        $this->wplogger->info(print_r($invoice->getEntityId(), true));
        ...
        if ($orderIncrementId) {
            ...
            if ($collectionData) {
                ...
                if (floatval($invoice->getGrandTotal()) != floatval($order->getGrandTotal())) {
                    $amount = $this->_amountAsInt($invoice->getGrandTotal());
                    $currency = $invoice->getOrderCurrencyCode();
                    $data = [];
                    $data['value'] = ['amount' => $amount, 'currency'=>$currency];
                    $data['reference'] = 'Partial-capture-for-'.$order->getIncrementId();
                    $data = json_encode($data);
                    $response = $this->sendRequest($partialCaptureUrl, $userName, $password, $data);
                } else {
                    $response = $this->sendRequest($captureUrl, $userName, $password);
                }
                if ($response) {
    ...
    public function sendRequest($captureUrl, $username, $password, $data = null)
    {
        $request = $this->_getRequest();
        $request->setUrl($captureUrl);
...
//         ));
        if ($data) {
            ...
            $this->wplogger->info('Sending Json as: ' . $captureUrl);
        }
